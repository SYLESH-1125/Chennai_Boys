...
=======
    .select('quizcode, score, correct_answers, total_questions, time_spent, submittedat, answers')
...
<<<<<<< HEAD
  ...
  const { data: quizzes, error: quizzesError } = await supabase
    ...
    .in('id', quizIds);
=======
  // Fetch all quiz titles for the quiz codes
  const quizCodes = [...new Set(results.map((r: any) => r.quizcode))];
  const { data: quizzes, error: quizzesError } = await supabase
    .from('quizzes')
    .select('code, title')
    .in('code', quizCodes);
...
=======
  // Map quizcode to title
  const quizCodeToTitle: Record<string, string> = {};
  quizzes.forEach((q: any) => {
    quizCodeToTitle[q.code] = q.title;
  });
  // Build quiz_history array with titles
  const quizHistory = results.map((r: any) => ({
    quiz_code: r.quizcode,
    title: quizCodeToTitle[r.quizcode] || r.quizcode,
>>>>>>> 13b50ce (Updated Faculty and some Student's Functionality)
    score: r.score,
