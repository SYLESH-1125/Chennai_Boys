-- =====================================================
-- CORRECTED COMPLETE DATABASE UPDATE SCRIPT
-- =====================================================
...
-- Note: First check what the actual column names are, then create indexes
-- Create indexes only if the columns exist to avoid errors

-- Check if students table exists and create index
DO $$
BEGIN
...
-- Check quiz_results table structure first before creating index
DO $$
BEGIN
    -- Check if quiz_results table exists and has studentid column
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'quiz_results' AND column_name = 'studentid'
    ) THEN
        CREATE INDEX IF NOT EXISTS idx_quiz_results_student_quiz ON quiz_results(studentid, quiz_id);
    -- If it uses student_id instead
    ELSIF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'quiz_results' AND column_name = 'student_id'
    ) THEN
...
-- =====================================================
-- CHECK TABLE STRUCTURES (OPTIONAL - FOR REFERENCE)
-- =====================================================

-- Uncomment these if you want to see the actual table structures
-- SELECT 'quiz_results table structure:' as info;
-- SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'quiz_results';
