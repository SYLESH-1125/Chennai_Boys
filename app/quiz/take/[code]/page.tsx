...
=======
        studentid: user.id,
        quizcode: quiz.code, // Use quiz.code instead of quiz.id
        score: scorePercent, // store percent
        attempts: 1,
        status: "completed",
        submittedat: new Date().toISOString(), // Use submittedat instead of submitted_at
>>>>>>> 13b50ce (Updated Faculty and some Student's Functionality)
...
=======
                {/* Render question with blanks replaced by unique inputs */}
                {(() => {
                  // Replace ___ or {blank} in question text with input
                  ...
                  return parts.map((part: string, idx: number) => {
                    if (BLANK_REGEX.test(part)) {
                      const currentBlankIndex = blankIndex++;
                      const answerKey = `${currentQ.id}_blank_${currentBlankIndex}`;
                      
                      return (
                        <Input
                          key={`blank-${currentQ.id}-${currentBlankIndex}`}
                          value={answers[answerKey] || ""}
                          onChange={e => {
                            // Handle multiple blanks by storing each blank separately
                            const newAnswers = { ...answers };
                            newAnswers[answerKey] = e.target.value;
                            
                            // Also update the main answer as a combined string for compatibility
                            const allBlanks = [];
                            for (let i = 0; i <= currentBlankIndex; i++) {
                              allBlanks.push(newAnswers[`${currentQ.id}_blank_${i}`] || "");
                            }
                            newAnswers[currentQ.id] = allBlanks.join(" | ");
