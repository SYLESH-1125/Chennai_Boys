...
=======
  ...
  // Edit mode state
  const editQuizCode = searchParams.get('edit')
  const isEditMode = !!editQuizCode
  const [originalQuizId, setOriginalQuizId] = useState<string>("")
>>>>>>> 13b50ce (Updated Faculty and some Student's Functionality)
...
=======
  ...
  useEffect(() => {
    const loadQuizForEdit = async () => {
      if (!isEditMode || !editQuizCode || !user) return
...
=======
    try {
      if (isEditMode && originalQuizId && editQuizCode) {
        ...
        if (updateError) {
          setError("Failed to update quiz: " + updateError.message)
          setIsSaving(false)
          return
        }
...
=======
                      ...
                      <SelectItem value="Design and Analysis Of Algorithms">Design and Analysis Of Algorithms</SelectItem>
